...
BPlusNode* insertIntoTree (BPlusNode* root, int key, void* value, size_t value_size)
{
    ...
    record_pointer = createRecord(value, value_size);
...
record* createRecord(void* value, size_t value_size) 
{
    // This line gets rid of the unused parameter warning, value_size may be relavent if dynamical memery allocation for value is desired.
    ...
    else 
    {
        /*
        printf("step 1.2.3\n");
        new_record->value = malloc(value_size);
        printf("step 1.2.35\n");
        memcpy(new_record->value, value, value_size);
        printf("step 1.2.4\n");
        */
